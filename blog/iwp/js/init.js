...
$(function () {
	...
	$('.update_view_dropdown').live('change',function(){
		$(".item_ind").not(".vulnurable_par").show().addClass("active");
		$(".row_updatee").not(".notVul").show().addClass("active");
		$(".row_updatee").not(".notHide").show().addClass("active");
		$(".item_ind").not(".hiddenView").show().addClass("active");
		var value = $(this).val();
		...
		if(scheduleAddonFlag==1){
			if(isScheduledBackup){
				...
				if(sType=='daily'){
					...
					valArray['scheduleHR']=sTime+"|"+$("#backupOptions .date_select_cont .selectDate.active").text();
				}
				if($(this).attr('schedulekey')!=undefined && $(this).attr('schedulekey')!=''){
...
$(".toolbar_sites_cont.overflowTabs").live('click',function(){
	...
	$(".updateActionBtn").live('click',function() { 
		...
		if($(this).attr('btnAction')=="check"){
			...
			tempDataArray['requiredData']['isAddonSuiteMiniCancelMessage']=1;
	...
	$("#maintenanceSiteConfirm").live('click',function(){
		...
		tempArray['args']['params']['mHTML']=$("#maintenanceHTML").val();
		tempArray['args']['siteIDs'][0]=$(this).attr('sid');
		tempArray['requiredData'] = {};
	...
	if(isShow2_7AddonUpdatePopup == true){
		var content='<div class="dialog_cont update_client_plugin"> <div class="th rep_sprite"> <div class="title droid700">Whats new?</div> </div> <div style="padding:20px;"><div style="text-align:center;line-height: 20px;"><div class="clear-both"></div> <div><span class="droid700">V2.7</span><ul><li>Compatible with PHP7</li><li> Login log</li><li>Upload plugin/theme and group items to My Favorites</li><li>Send email when backup schedule fails</li><li>Security enhancement for SQL wrapper</li></ul></div></div><div class="clear-both"></div><div style="text-align: center; padding-top: 10px;">&bull;</div><div class="" style="padding-top: 10px;    text-align: center; line-height: 22px;">Try out the new features and be sure to report any errors you face.</div></div> <div style="background-color: #EFDEDE;border-left: 2px solid #8A1010;padding: 10px; margin: 10px;line-height: 18px;">Important: You have to update all addons for the Security enhancement for SQL wrapper to take full effect. <a id="close2_7AddonUpdatePopup">Update all addons now</a></div><div class="clear-both"></div> <div class="th_sub rep_sprite" style="border-top:1px solid #c6c9ca;"><div class="btn_action float-right"><a class="rep_sprite" id="close2_7AddonUpdatePopup">Okay</a></div></div>';
		$("#modalDiv").dialog("destroy");
		$('#modalDiv').html(content).dialog({width:'auto',modal:true,closeOnEscape:false,position: 'center',resizable: false, open: function(event, ui) { bottomToolBarHide(); },close: function(event, ui) {bottomToolBarShow(); }});
	}
	$("#close2_7AddonUpdatePopup").live("click", function(){
		var tempArray={};
		tempArray['requiredData']={};
		tempArray['requiredData']['close2_7AddonUpdatePopup']=1;
		doCall(ajaxCallPath,tempArray,'process2_7AddonUpdatePopup');
	});


	$( '#bottom_sites_cont #bottom_left .list_cont .content').scroll(function() {
		lazyLoadOnlyVisibleImages();
	}); 
